...
describe('MoviesApiService', () => {
  ...
  it('should return top three studios with wins by default', () => {
    ...
    service.getTopStudiosWithWins().subscribe((studios) => {
  ...
  it('should return top custom studios with wins', () => {
    const mockResponse: Studios = {
      studios: [
        { name: 'Studio A', winCount: 10 },
        { name: 'Studio B', winCount: 8 },
    ...
    service.getTopStudiosWithWins(4).subscribe((studios) => {
      expect(studios.length).toBe(4);
      expect(studios).toEqual(mockResponse.studios.slice(0, 4));
    });
  ...
  it('should handle error in getTopThreeStudiosWithWins', () => {
    ...
    spyOn(service, 'getTopStudiosWithWins').and.returnValue(
      throwError(() => mockError)
    );

    service.getTopStudiosWithWins().subscribe({
      next: () => {
